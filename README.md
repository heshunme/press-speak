...
- 首次运行可自动下载、解压并校验模型
- 最终文本支持离线标点和可配置后处理规则
- 默认将明确的中文数值转成阿拉伯数字，并将全大写普通英文转小写；API、GPT、ChatGPT 等按可编辑词表保留标准拼写
...
在设置的“后处理规则”页可分别关闭“数字规范化”和“英文大小写规范化”，也可输入文本测试并查看命中的规则。
例如“一百二十三个”变为“123个”，“数值是负三点五”变为“数值是-3.5”，“编号零零一二”变为“编号0012”；
“HELLO WORLD”变为“hello world”，“G P T”变为“GPT”。英文术语词表每行一个标准拼写，也支持逗号分隔；清空词表后仅保留独立的英文代词 `I`。
已有标题式/混合大小写、网址、邮箱及可识别的路径和代码标识符保留原样。数字采用保守转换，日期、时间、分数、约数和省略位值（如“一百二”）暂不转换。
“三五分钟”“一起”“周一”保持原样；小数空格只在明确数值上下文（如“数值是3. 14”）或独立且点前有空格的表达式中清理，避免合并句号和列表编号。
